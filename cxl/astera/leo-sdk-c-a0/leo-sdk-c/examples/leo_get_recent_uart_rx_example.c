...
/**
 * @file leo_get_recent_uart_rx_example.c
...
void doLeoGetRecentUartRxExample(LeoDeviceType *leoDevice);
...
int main(int argc, char *argv[]) {
  ...
  if (defaultArgs.bdf != NULL) {
    ...
    while (nextbdf != NULL) {
      ...
      doLeoGetRecentUartRxExample(leoDevice);
      nextbdf = strtok(NULL, ",");
    }
  } else {
...
void doLeoGetRecentUartRxExample(LeoDeviceType *leoDevice) {
  ...
  int rc;
  char recent_uart_rx;
  rc = leoGetRecentUartRx(leoDevice, &recent_uart_rx);
  if (rc == 0) {
    ASTERA_INFO("recent UART input: %c", recent_uart_rx);
  }
  else {
    ASTERA_INFO("no UART input detected");
  }
}
