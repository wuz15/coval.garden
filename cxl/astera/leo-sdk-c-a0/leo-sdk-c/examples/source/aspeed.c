...
int asteraI2CWriteBlockData(int handle, int cmdCode, uint8_t numBytes,
                            uint8_t *buf) {
  return i2c_smbus_write_i2c_block_data(handle, cmdCode, numBytes, buf);
}

int asteraI2CReadBlockData(int handle, int cmdCode, uint8_t numBytes,
                           uint8_t *buf) {
 ...
 * Close I2C handle
 */
void asteraI2CCloseConnection(int file) { close(file); }
