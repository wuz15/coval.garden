...
void asteraLogSetUdata(void *udata) { AsteraLogger.udata = udata; }

void asteraLogSetLock(logLockFn fn) { AsteraLogger.lock = fn; }

void asteraLogSetFp(FILE *fp) { AsteraLogger.fp = fp; }
...
void asteraLogSetQuiet(int enable) { AsteraLogger.quiet = enable ? 1 : 0; }
...
void asteraLogMsg(int level, const char *file, int line, const char *fmt, ...) {
  ...
  if (AsteraLogger.fp) {
    ...
    fprintf(AsteraLogger.fp, "%s %-5s %s:%d: ", buf, logLevelNames[level], file,
            line);
    va_start(args, fmt);
